...
public class CustomGraphicalViewerKeyHandler extends KeyHandler {
    ...
    Point getNavigationPoint( IFigure figure ) {
        if ( figure instanceof AttributeNameFigure || figure instanceof AttributeValueFigure )
     ...
     * @param direction PositionConstants.* indicating the direction in which to traverse
     */
    boolean navigateNextSibling( KeyEvent event, int direction, List<GraphicalEditPart> siblingParts ) {
        GraphicalEditPart epStart = getFocusEditPart();
        GraphicalEditPart result = NodeContentsKeyNavigator.navigate( epStart, direction, siblingParts );
        if ( result != null ) {
            navigateTo( result, event );
            return true;
        }
        // if (event == PositionConstants.
        IFigure figure = epStart.getFigure();
        Point pStart = getNavigationPoint( figure );
        figure.translateToAbsolute( pStart );
        EditPart next = findSibling( siblingParts, pStart, direction, epStart );
        if ( next == null )
            return false;
        navigateTo( next, event );
        return true;
